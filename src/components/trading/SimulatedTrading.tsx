...
const mockOptionsData = {
  AAPL: { 
    standard: { bid: 10, ask: 11, strike: 150 },
    leaps: [
      { bid: 32.5, ask: 34.0, strike: 200, expiry: "Jan 2026", iv: 18.5, delta: 0.65, theta: -0.045 },
      { bid: 48.7, ask: 50.2, strike: 250, expiry: "Jan 2026", iv: 20.2, delta: 0.72, theta: -0.038 },
      { bid: 29.3, ask: 30.6, strike: 300, expiry: "Jun 2026", iv: 22.4, delta: 0.58, theta: -0.032 },
      { bid: 58.4, ask: 60.1, strike: 280, expiry: "Jan 2027", iv: 16.8, delta: 0.78, theta: -0.025 }
    ]
  },
  SPY: { 
    standard: { bid: 8.5, ask: 9.2, strike: 450 },
    leaps: [
      { bid: 45.2, ask: 46.8, strike: 480, expiry: "Jan 2026", iv: 15.8, delta: 0.61, theta: -0.042 },
      { bid: 63.5, ask: 65.2, strike: 550, expiry: "Jan 2026", iv: 18.5, delta: 0.56, theta: -0.036 },
      { bid: 38.9, ask: 40.3, strike: 600, expiry: "Jun 2026", iv: 21.2, delta: 0.45, theta: -0.029 },
      { bid: 75.6, ask: 77.3, strike: 520, expiry: "Jan 2027", iv: 16.5, delta: 0.69, theta: -0.022 }
    ]
  },
  TSLA: { 
    standard: { bid: 15.2, ask: 16.0, strike: 240 },
    leaps: []
  },
  MSFT: { 
    standard: { bid: 12.5, ask: 13.1, strike: 325 },
    leaps: []
  },
  QQQ: { 
    standard: { bid: 13.8, ask: 14.5, strike: 400 },
    leaps: [
      { bid: 42.6, ask: 44.2, strike: 420, expiry: "Jan 2026", iv: 17.2, delta: 0.58, theta: -0.040 },
      { bid: 56.8, ask: 58.4, strike: 500, expiry: "Jan 2026", iv: 19.8, delta: 0.52, theta: -0.035 },
      { bid: 35.4, ask: 37.1, strike: 550, expiry: "Jun 2026", iv: 22.3, delta: 0.41, theta: -0.027 },
      { bid: 68.5, ask: 70.2, strike: 480, expiry: "Jan 2027", iv: 15.9, delta: 0.64, theta: -0.020 }
    ]
  }
};
...
// Expiry types
const expiryTypes = [
  { value: "standard", label: "Standard (30-90 days)" },
  { value: "leaps", label: "LEAPS (Long-term)" },
];
...
const SimulatedTrading = () => {
  ...
  useEffect(() => {
    if (expiryType === "leaps") {
      ...
      const matchingOption = leapsOptions.find(
        option => option.expiry === leapsExpiry && option.strike === parseFloat(strikePrice)
      );
      ...
      if (matchingOption) {
        ...
        const closestOption = leapsOptions
          .filter(option => option.expiry === leapsExpiry)
          .sort((a, b) => Math.abs(a.strike - parseFloat(strikePrice)) - Math.abs(b.strike - parseFloat(strikePrice)))[0];
  ...
  const getAvailableStrikes = () => {
    if (expiryType === "standard") {
      ...
      return mockOptionsData[selectedTicker]?.leaps
        .filter(option => option.expiry === leapsExpiry)
  ...
  const calculateEstimatedPnL = () => {
    ...
    const projectedChange = optionType === "call" 
      ? Math.max(0, stockPrice * 1.1 - parseFloat(strikePrice)) * 100 * qty 
      : Math.max(0, parseFloat(strikePrice) - stockPrice * 0.9) * 100 * qty;
  ...
  const getRecommendedLeapsAlert = () => {
    return mockLeapsAlerts.find(alert => 
      alert.symbol === selectedTicker && 
      alert.itmProbability >= 65
    );
  };
                ...
                <div>
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      const closestExpiry = leapsExpiryDates.find(exp => 
                        exp.label.includes(recommendedAlert.expiryDate.split('/')[2])
                      )?.value || "Jan 2026";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              {expiryType === "leaps" ? (
                <Select 
                  value={strikePrice} 
                  onValueChange={(value) => setStrikePrice(value)}
                >
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between pt-2 border-t border-muted/20">
                  ...
                  <span className={cn(
                    "font-semibold",
                    expiryType === "leaps" ? "text-emerald-400" : "text-optionpulse-blue"
                  )}>${calculateTotalCost().toLocaleString()}</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-muted/20">
                  ...
                  <div 
                    ...
                    className={cn(
                      ...
                      trade.isLeaps 
                        ...
                        : "border-optionpulse-blue/30 bg-optionpulse-blue/5"
                    )}
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
