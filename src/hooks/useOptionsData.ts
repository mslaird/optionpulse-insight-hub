      ...
      try {
        ...
        if (typeof jsonData === 'object' && jsonData !== null) {
          ...
          if (Array.isArray(typedData.options) && Array.isArray(typedData.strategies)) {
            // Create a properly formed OptionsChainData object
            const baseResult = {
            ...
            if (expirationDate) {
              ...
              const filteredOptions = baseResult.options.filter(option => 
                option.expiryDate === expirationDate
              );
              console.log(`Filtered ${filteredOptions.length} options out of ${baseResult.options.length}`);
              
              // Also filter strategies that match this expiration date
              const filteredStrategies = baseResult.strategies.filter(strategy => 
                // Check if any leg of the strategy has the matching expiry date
            ...
            console.log("Processed data:", baseResult);
            return baseResult;
          }
...
const createMockOptionsData = (symbol: string, expirationDate?: string): OptionsChainData => {
  const currentDate = new Date();
  const expiryDate = expirationDate || new Date(currentDate.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]; // Default 30 days out
